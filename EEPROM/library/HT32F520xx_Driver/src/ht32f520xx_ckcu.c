...
{
  CKCU_BF_WRITE(HT_CKCU->GCFGR, CKCU_MASK_PLLSRC, CKCU_POS_PLLSRC, PLLInit->ClockSource);
  CKCU_BF_WRITE(HT_CKCU->PLLCR, CKCU_MASK_PLLBYPASS, CKCU_POS_PLLBYPASS, PLLInit->BYPASSCmd);
  HT_CKCU->PLLCFGR = PLLInit->CFG;
}
...
void CKCU_GetClocksFrequency(CKCU_ClocksTypeDef* CKCU_Clk)
{
  ...
  if (div >= 5)
  {
    CKCU_Clk->HCLK_Freq = (CKCU_Clk->SYSCK_Freq)>>5;
  }
  else
  {
    CKCU_Clk->HCLK_Freq =(CKCU_Clk->SYSCK_Freq) >> div;
  }
  ...
  if (div == 7)
  {
    CKCU_Clk->ADC_Freq = (CKCU_Clk->HCLK_Freq) / 3;
  }
  else
  {
 ...
 *             CKCU_PCLK_BFTM0, CKCU_PCLK_BFTM1, CKCU_PCLK_MCTM0,
 ...
 *             CKCU_PCLK_WDTR,  CKCU_PCLK_BKPR,  CKCU_PCLK_SCI0,   CKCU_PCLK_SCI1,
 *             CKCU_PCLK_I2S,   CKCU_PCLK_SCTM0, CKCU_PCLK_SCTM1,
 ...
 *        @arg CKCU_APBCLKPRE_DIV16: specific peripheral clock = PCLK / 16 (BKPRCLK)
 *        @arg CKCU_APBCLKPRE_DIV32: specific peripheral clock = PCLK / 32 (BKPRCLK)
 * @retval None
 ************************************************************************************************************/
 ...
 *        This parameter can be:
 *        @arg CKCU_AHBEN_SLEEP_FMC, CKCU_AHBEN_SLEEP_SRAM, CKCU_AHBEN_SLEEP_BM, CKCU_AHBEN_SLEEP_APB
 * @param Cmd: This parameter can be ENABLE or DISABLE.
 * @retval None
 ************************************************************************************************************/
 ...
 *             CKCU_DBG_MCTM0_HALT,  CKCU_DBG_GPTM0_HALT,  CKCU_DBG_GPTM1_HALT,
 *             CKCU_DBG_SCTM0_HALT,  CKCU_DBG_SCTM1_HALT,  CKCU_DBG_SCTM2_HALT,  CKCU_DBG_SCTM3_HALT,
 *             CKCU_DBG_BFTM0_HALT,  CKCU_DBG_BFTM1_HALT,
 *             CKCU_DBG_USART0_HALT, CKCU_DBG_USART1_HALT, CKCU_DBG_UART0_HALT,  CKCU_DBG_UART1_HALT,
