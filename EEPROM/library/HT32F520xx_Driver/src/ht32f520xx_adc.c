 ...
 * @param HT_ADCn: where HT_ADCn is the selected ADC from the ADC peripherals.
 * @param NewState: new state of the HT_ADCn peripheral.
...
void ADC_Cmd(HT_ADC_TypeDef* HT_ADCn, ControlStatus NewState)
{
  ...
  if (NewState != DISABLE)
  {
    SetBit_BB((u32)&HT_ADCn->CR, 7);
  }
  else
  {
    ResetBit_BB((u32)&HT_ADCn->CR, 7);
  }
}
 ...
 * @param HT_ADCn: where HT_ADCn is the selected ADC from the ADC peripherals.
 * @param ADC_CH_x: the ADC channel to configure
     This parameter can be one of the following values:
       @arg ADC_CH_x: ADC Channel x selected, x must between 0 ~ 11
...
void ADC_RegularChannelConfig(HT_ADC_TypeDef* HT_ADCn, u8 ADC_CH_x, u8 Rank)
{
  ...
  /* Check the parameters                                                                                   */
  Assert_Param(IS_ADC(HT_ADCn));
  Assert_Param(IS_ADC_CHANNEL(ADC_CH_x));
  ...
  tmpreg2 = (u32)ADC_CH_x << (8 * (Rank & 0x3));
...
/*********************************************************************************************************//**
 * @brief Configure the HT_ADCn trigger source for regular channels conversion.
 * @param HT_ADCn: where HT_ADCn is the selected ADC from the ADC peripherals.
 * @param ADC_TRIG_x:
     This parameter can be one of the following values:
...
/*********************************************************************************************************//**
 * @brief Clear the HT_ADCn interrupt pending bits.
 * @param HT_ADCn: where HT_ADCn is the selected ADC from the ADC peripherals.
 ...
 * @param HT_ADCn: where HT_ADCn is the selected ADC from the ADC peripherals.
 * @param ADC_PDMA_REGULAR_x: Specify the ADC PDMA request that is to be enabled or disabled.
     This parameter can be any combination of the following values:
...
void ADC_PDMAConfig(HT_ADC_TypeDef* HT_ADCn, u32 ADC_PDMA_REGULAR_x, ControlStatus NewState)
{
  /* Check the parameters                                                                                   */
  Assert_Param(IS_ADC(HT_ADCn));
  Assert_Param(IS_ADC_PDMA(ADC_PDMA_REGULAR_x));
  Assert_Param(IS_CONTROL_STATUS(NewState));

  if (NewState != DISABLE)
  {
    HT_ADCn->PDMAR |= ADC_PDMA_REGULAR_x;
  }
  else
  {
    HT_ADCn->PDMAR &= ~ADC_PDMA_REGULAR_x;
